...
AWSへのデプロイは完了しており、既存のLINEチャンネルからBookmarkを登録できることまで確認済みです。Android側では、下記のCognito設定値とAPI URLをビルド設定や環境別設定へ切り出してください。
...
| 項目 | 値・取得方法 | 状態 |
|---|---|---|
| Region | `ap-northeast-3` | CloudFormation出力で確認済み |
| User Pool ID | `ap-northeast-3_H9F0jf3UU` | CloudFormation出力で確認済み |
| API Base URL | `https://gctpao66n5.execute-api.ap-northeast-3.amazonaws.com/prod` | Bookmark APIをデプロイ済み |
| Android Client ID | `31gsafnpolljr8duqle36giqjc` | CloudFormation出力で確認済み |
| Hosted UI Domain | `https://todo-app-010526284951.auth.ap-northeast-3.amazoncognito.com` | CloudFormation出力で確認済み |
| Callback URL | `todobookmark://callback` | Cognitoへ登録済み |
| Sign-out URL | `todobookmark://signout` | Cognitoへ登録済み |

再デプロイ時は、`ApiUrl`、`UserPoolId`、`AndroidUserPoolClientId`、`HostedUiDomain`、`Region`、`AndroidCallbackUrl`の各出力が変わっていないか確認してください。
